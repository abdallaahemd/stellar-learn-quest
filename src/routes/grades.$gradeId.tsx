            ...
            >
              <GlassCard glow={grade.accent} className="h-full">
                <div className="flex items-start justify-between">
                  <div className="grid h-14 w-14 place-items-center rounded-2xl bg-white/5 text-2xl">
                    {m.emoji}
                  </div>
                ...
                {/* progress visual (placeholder bar) */}
                <div className="mt-6 h-1.5 w-full overflow-hidden rounded-full bg-white/5">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${((i + 1) / (grade.modules.length + 1)) * 100}%` }}
